 ...
 *
 */
class WebGLDebugDrawManager implements DebugDrawManager {
  ...
  WebGLDebugDrawManager(this.device, {int maxVertices: 16384}) {
   ...
   */
  void drawLine(Vector3 start, Vector3 finish, Vector4 color,
   ...
   */
  void drawCross(Vector3 point, Vector4 color,
                num size, num duration, bool depthEnabled) {
    var lineManager = depthEnabled ? _depthEnabledLines : _depthDisabledLines;
    lineManager.lines.startLineObject(color.r, color.g, color.b, color.a,
                                      duration);
   ...
   */
  void drawSphere(Vector3 center, num radius, Vector4 color,
                 num duration, bool depthEnabled) {
    var lineManager = depthEnabled ? _depthEnabledLines : _depthDisabledLines;
    lineManager.lines.startLineObject(color.r, color.g, color.b, color.a,
                                      duration);
    ...
    for (int y = 0; y < latSegments; ++y) {
      ...
      for (int x = 0; x <= lonSegments; ++x) {
        ...
        vertex = new Vector3(
            radius * cos(u * twoPi) * sin(v * PI),
            radius * cos(v * PI),
            radius * sin(u * twoPi) * sin(v * PI)
        ) + center;

        upperVertex = new Vector3(
            radius * cos(u * twoPi) * sin(v2 * PI),
            radius * cos(v2 * PI),
            radius * sin(u * twoPi) * sin(v2 * PI)
        ) + center;
  ...
  void drawCone(Vector3 apex, Vector3 direction, num height, num angle,
    ...
    num s = height / cos(angle / 2.0);
    num radius = sqrt(s * s - height * height);
  ...
  void render(Matrix4 viewProjectionMatrix) {
    Matrix4 pm = viewProjectionMatrix;
    //Matrix4 la = viewMatrix;
    //pm.multiply(la);
